...
public class JuizUDP {
    ...
    private static DatagramSocket servidorSocketGlobal; // Variável estática global
    ...
    public static void configurarBarreira() {
        ...
        long jogadoresEmJogo = jogadoresConectados.values().stream().filter(j -> j.emJogo).count();
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            servidorSocketGlobal = serverSocket; // Atribui a instância local ao campo estático global
                ...
                poolDeThreads.submit(() -> {
                    ...
                    if (jogadorAtual == null) {
                        ...
                        try {
                            ...
                            // Não é um número, pode ser nickname
                        }

                        if (!mensagemRecebida.isEmpty() && !ehNumero) {
                            synchronized (jogadoresConectados) {
                                if (jogadoresConectados.containsKey(mensagemRecebida)) {
                                    String msgErroCadastro = "Desculpe, este nickname já está em uso.";
                                    try {
                                        ...
                                        // AQUI: Usar servidorSocketGlobal
                                        servidorSocketGlobal.send(pacoteErro); 
                                    } catch (IOException e) {
                                        System.err.println("Erro ao enviar msg de erro de nickname: " + e.getMessage());
                                    }
                                    return;
                                }
                            ...
                            String msgErroCadastro = "Desculpe, a entrada para cadastro é inválida (vazia ou é um número).";
                            try {
                                byte[] dadosMsg = msgErroCadastro.getBytes();
                                DatagramPacket pacoteErro = new DatagramPacket(dadosMsg, dadosMsg.length, enderecoCliente, portaCliente);
                                // AQUI: Usar servidorSocketGlobal
                    ...
                    } else { // Mensagem de um jogador já cadastrado.
                        try {
                            ...
                            switch (escolha) {
                                case 1: // Ver regras
                                    ...
                                    try {
                                        // AQUI: Usar servidorSocketGlobal
                                        jogadorAtual.enviarMensagem(servidorSocketGlobal, regras);
                                    } catch (IOException e) {
                                        System.err.println("Erro ao enviar regras: " + e.getMessage());
                                    }
                                    ...
                                    try {
                                        ...
                                        System.err.println("Erro ao reenviar menu (regras): " + e.getMessage());
                                    }
                                    break;
                                ...
                                case 2: // INICIAR O JOGO
                                    if (!jogadorAtual.emJogo) {
                                        if (semaforoPartida.tryAcquire()) {
                                            ...
                                            if (jogadoresAtivosNaPartida == N_JOGADORES_INICIAIS) {
                                                ...
                                                synchronized (jogadoresConectados) {
                                                    for (Jogador j : jogadoresConectados.values()) {
                                                        if (j.emJogo) {
                                                            try {
                                                                // AQUI: Usar servidorSocketGlobal
                                                                j.enviarMensagem(servidorSocketGlobal, "Jogadores oponentes encontrados. Que comecem os jogos...");
                                                                j.enviarMensagem(servidorSocketGlobal, "Escolha um número entre 0 e 100:");
                                                            } catch (IOException e) {
                                                                System.err.println("Erro ao enviar início de jogo: " + e.getMessage());
                                                            }
                                                        }
                                                    }
                                                }
                                                ...
                                                try {
                                                    ...
                                                    System.err.println("Erro ao enviar aguardando jogadores: " + e.getMessage());
                                                }
                                            }
                                        } else {
                                            ...
                                            try {
                                                ...
                                                System.err.println("Erro ao enviar jogo cheio: " + e.getMessage());
                                            }
                                        }
                                    } else {
                                        ...
                                        try {
                                            ...
                                            System.err.println("Erro ao enviar já em jogo: " + e.getMessage());
                                        }
                                    }
                                    break;
                                ...
                                case 3: // Sair do jogo
                                    ...
                                    try {
                                        ...
                                        System.err.println("Erro ao enviar mensagem de saída: " + e.getMessage());
                                    }
                                ...
                                default: // Para qualquer outro número, tenta interpretar como uma jogada.
                                    if (jogadorAtual.emJogo && escolha >= 0 && escolha <= 100) {
                                        ...
                                        try {
                                            if (barreiraRodada != null) {
                                                barreiraRodada.await(); // Esta é a chamada BLOQUEANTE, mas agora dentro de uma thread do pool.
                                            } else {
                                            ...
                                            try {
                                                ...
                                                System.err.println("Erro ao enviar erro de barreira: " + eIO.getMessage());
                                            }
                                            mensagemParaJogador = "Bem-vindo(a)," + jogadorAtual.nickname + ".\n" +
                                                                  "Digite 1 - para ver as regras do jogo.\n" +
                                                                  "Digite 2 - para iniciar o jogo.\n" +
                                            ...
                                            try {
                                                ...
                                                System.err.println("Erro ao reenviar menu (barreira erro): " + eIO.getMessage());
                                            }
                                        }
                                    } else {
                                        mensagemParaJogador = "Entrada inválida. Digite um número de 0 a 100 (se o jogo começou) ou escolha uma opção de menu.";
                                        try {
                                            // AQUI: Usar servidorSocketGlobal
                                            jogadorAtual.enviarMensagem(servidorSocketGlobal, mensagemParaJogador);
                                        } catch (IOException e) {
                                            System.err.println("Erro ao enviar entrada inválida: " + e.getMessage());
                                        }
                                        mensagemParaJogador = "Bem-vindo(a)," + jogadorAtual.nickname + ".\n" +
                                                              "Digite 1 - para ver as regras do jogo.\n" +
                                                              "Digite 2 - para iniciar o jogo.\n" +
                                        ...
                                        try {
                                            ...
                                            System.err.println("Erro ao reenviar menu (entrada inválida): " + e.getMessage());
                                        }
                                    }
                                    break;
                            }
                            ...
                            try {
                                ...
                                System.err.println("Erro ao enviar entrada inválida (NumberFormatException): " + eIO.getMessage());
                            }
                            mensagemParaJogador = "Bem-vindo(a)," + jogadorAtual.nickname + ".\n" +
                                                 "Digite 1 - para ver as regras do jogo.\n" +
                                                 "Digite 2 - para iniciar o jogo.\n" +
                            ...
                            try {
                                ...
                                System.err.println("Erro ao reenviar menu (NumberFormatException): " + eIO.getMessage());
                            }
                        }
                    }
        ...
        for (Jogador jogador : jogadoresAtivosNaRodada) {
            ...
            if (jogador.pontuacao <= -6) {
                try {
                    ...
                    System.err.println("Erro ao enviar mensagem de eliminação: " + e.getMessage());
                }
        ...
        if (jogadoresAtualmenteEmJogo == 1) {
            ...
            if (vencedor != null) {
                try {
                    ...
                    System.err.println("Erro ao enviar mensagem de vitória: " + e.getMessage());
                }
            ...
            synchronized (jogadoresConectados) {
                for (Jogador j : jogadoresConectados.values()) {
                    ...
                    try {
                        ...
                        System.err.println("Erro ao enviar mensagem de partida encerrada: " + e.getMessage());
                    }
                }
            }
                ...
                for (Jogador jogador : jogadoresConectados.values()) {
                    if (jogador.emJogo) {
                        try {
                            ...
                            System.err.println("Erro ao enviar pedido de escolha de número: " + e.getMessage());
                        }
                    }
                }
            }
